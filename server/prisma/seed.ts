/**
 * Seed script — generates a realistic company with multi-year history so the
 * dashboards, charts and history cards are populated out of the box.
 *
...
async function main() {
  ...
  await prisma.careerGrowthMilestone.deleteMany();
  await prisma.financialGrowthMetric.deleteMany();
  ...
  const accounts: { email: string; password: string; role: string; code: string }[] = [
    { email: 'admin@demo.com', password: 'demo1234', role: 'ADMIN', code: 'E001' },
    { email: 'hr@demo.com', password: 'demo1234', role: 'HR', code: 'E002' },
    { email: 'manager@demo.com', password: 'demo1234', role: 'MANAGER', code: 'E004' },
    { email: 'employee@demo.com', password: 'demo1234', role: 'EMPLOYEE', code: 'E005' },
  ];
  for (const a of accounts) {
    await prisma.user.create({
      data: {
        ...
        role: a.role,
        employeeId: idByCode.get(a.code) ?? null,
      },
    });
  }
  ...
  console.log('\n✓ Seed complete:', JSON.stringify(counts, null, 2));
